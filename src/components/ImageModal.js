...
const ImageModalContent = styled.div`
  ...
  animation: ${({ imageLeft, imageTop, imageWidth }) =>
      imageAnimation(imageLeft, imageTop, imageWidth)}
    ${MODAL_OPEN_DURATION} ease forwards;
  border-radius: 15px;
...
const ModalImage = styled.img`
  ...
  animation: ${(props) =>
    props.isActive
      ? css`
          ${imageBorderAnimation} .3s ${MODAL_OPEN_DURATION} ease forwards
        `
      : "none"};
`;
...
const DescriptionBox = styled.div`
  ...
  max-height: 0;
  width: 100%;
  animation: ${descriptionDropDownAnimation} 0.3s ${MODAL_OPEN_DURATION} ease-in
    forwards;
