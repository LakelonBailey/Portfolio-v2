...
const App = () => {
  useEffect(() => {
    ...
    for (let themeNumber of themeNumbers) {
      ...
      const themeColor = getComputedStyle(document.documentElement)
        .getPropertyValue(`--theme-${themeNumber}`)
        .trim();
  ...
  const defaultPage = searchParams.get("firstPage") || "hero";
  ...
  return (
    <PageTransitionProvider>
      {defaultPage === "hero" && <Landing />}
      <Layout pages={mainPages} setPage={setPage} currentPage={currentPage}>
        <Page
